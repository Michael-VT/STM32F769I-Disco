...
void MX_GPIO_Init(void)
{
  ...
  GPIO_InitStruct.Pin = Audio_INT_Pin|ARD_D4_Pin|ARD_D2_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOJ, &GPIO_InitStruct);

  /* Configure ESP8266 control pins as outputs and drive them HIGH
   * ARD_D7 (PJ3) - May be CH_PD or RST, must be HIGH for ESP8266 to work
   * ARD_D8 (PJ4) - May be alternate GPIO, drive HIGH
   * WIFI_RST (PJ14) - Reset pin, must be HIGH normally
   */
  GPIO_InitStruct.Pin = WIFI_RST_Pin|ARD_D8_Pin|ARD_D7_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOJ, &GPIO_InitStruct);

  // Drive all ESP8266 control pins HIGH to enable the module
  HAL_GPIO_WritePin(GPIOJ, WIFI_RST_Pin|ARD_D8_Pin|ARD_D7_Pin, GPIO_PIN_SET);
